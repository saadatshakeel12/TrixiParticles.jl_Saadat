...
floor_thickness = (num_layers-1) * particle_spacing
z_bottom = z_surface - floor_thickness

z_top = z_top_surface

# 3. Create the shape using the calculated bottom
floor_particles = RectangularShape(particle_spacing, (20, 20, num_layers),
                                   (-0.01, -0.01, z_bottom);
                                   density=floor_density)

mold_particles = RectangularShape(particle_spacing, (20, 20, num_layers),
        (-0.01, -0.01, z_top);
        density=floor_density)
...
# Use acceleration (not fixed velocity) to allow contact mechanics to respond
# Don't set: mold_particles.velocity[3, :] .= -0.05
...
t_compress = cylinder_height * 0.5 / 0.05  # time to compress 10% at 50mm/s = 2.4s
...
callbacks = CallbackSet(
    UpdateCallback(),
    SolutionSavingCallback(dt=0.005, prefix="molding_cfrp_3d"),
    StepsizeCallback(cfl=1.5),  # Reduced CFL for better contact resolution
